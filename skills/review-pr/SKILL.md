...
Run specialized code reviews, full test suites, and quality checks in parallel using agents, then fix issues and post a structured PR comment with results. This is the gate before `/merge` — it ensures the branch has been reviewed and verified.

## Input
"$ARGUMENTS" - Optional PR number or context. If empty, auto-detect from current branch.
...
3. Auto-detect the PR via `gh pr view --json number,url` on the current branch
4. If `$ARGUMENTS` contains a PR number, use that instead
5. If no PR exists on the current branch, stop:
   > No PR found on this branch. Run `/pr` first, then re-run `/review-pr`.
6. Save the PR number and URL for the comment step
...
#### Tests + Quality (baseline — ensures review-pr is self-contained)
...
#### Reviews (specialized, parallel with tests + quality)
...
**Why run tests alongside reviews?** This baseline run ensures review-pr is self-contained — it doesn't depend on trusting that prep-pr ran on another machine. Tests run in parallel with reviews so they don't add wall time.
...
4. If all checks passed and reviews found nothing -> skip to **Step 4: Durable Signal**
...
- All pass -> commit: `fix: address correctness review findings`. Push to PR branch (set `dangerouslyDisableSandbox: true` on the push — SSH is blocked by sandbox).
...
- All pass -> commit: `fix: address security review findings`. Push to PR branch.
...
- All pass -> commit: `fix: address quality review findings`. Push to PR branch.
- 3 iterations exhausted -> report remaining in Step 4.

**Note on GH Actions re-trigger:** When review-pr pushes fix commits, the push can re-trigger CI workflows. The GH Actions workflow should guard against recursive triggers (e.g., skip if the commit message matches `fix: address * review findings`). This is not a skill concern but document the risk for consuming projects.

### Step 4: Durable Signal — PR Comment
Post a structured comment on the PR via `gh pr comment`:
...
- **Commit-per-phase**: each fix phase commits and pushes separately for traceability
...
- **Durable signal**: PR comment persists the review results — visible to anyone reviewing the PR, survives session end
